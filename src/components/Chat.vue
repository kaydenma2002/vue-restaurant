<template>
  ...
  <div class="bg-white z-40">
    ...
    <form
        class="absolute bottom-0 w-full z-10 bg-white"
...
export default {
  ...
  mounted() {
    HTTPS.get("/user").then((res) => {
      ...
      this.recipient = 1
        ...
        this.channel = this.echo.private("private-chat." + this.sender + "." + this.recipient);
  ...
  methods: {
    ...
    async submit() {
      ...
      HTTPS.post("/private-messages", { message: this.message }).then((res) => {
        this.loading = false;
      });
