<template>
  <div
    class="relative h-2/3 md:w-1/2 lg:w-1/2 border rounded chat-box overflow-auto flex flex-col chat-popup z-10 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
    v-if="isOpen"
  >
    ...
    <div class="grid grid-cols-4 gap-4 h-full overflow-y-auto">
      <div class="overflow-y-auto col-span-1 mb-16">
        <ul class="" v-for="user in users" :key="user.id">
          <li>
            <button @click="loadMessage(user.id)">{{ user.name }}</button>
          </li>
        </ul>
      </div>
      <div class="col-span-3">
        <div class="w-full p-6" ref="chatContainer">
          <ul
            ...
            ref="scrollToMe"
          >
            <li v-if="sender == item.sender_id" class="flex justify-end">
              <div
        ...
        <div class="mb-20">
          <div class="text-center w-full">Let's chat together</div>
        </div>
      </div>
    </div>
    <form class="absolute bottom-0 w-full bg-white" @submit.prevent="submit">
      <div
...
export default {
  ...
  mounted() {
    ...
    this.channel = this.echo.private(
      "private-chat." + this.sender + "." + this.recipient
    );
  },
  methods: {
    togglePopup() {
    ...
    loadMessage(id) {
      ...
      this.channel.listen("PrivateChatEvent", (data) => {
        this.sent.push(data);
        console.log(data);
      });
      HTTPS.get("private-chat", {
        params: { recipient_id: this.recipient },
      }).then((res) => {
