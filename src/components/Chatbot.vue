<template>
  <div
    class="relative h-2/3 w-80 md:w-96 lg:w-96 border rounded chat-box overflow-auto flex flex-col chat-popup z-10 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
    v-if="isOpen"
  >
    ...
    <div class="mb-20">
      <div class="text-center w-full">What may I help you today ?</div>
  ...
  methods: {
    ...
    async submitbyChoices(e) {
      ...
      if (this.sent.filter((item) => item.includes("reservation")).length > 1) {
        ...
        this.sent.filter((item) => item.includes("order")).length > 1
      ) {
